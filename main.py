VERSION_LABEL = "v8_reminder_text_fixed"
...
def report_text_for_dates(start_date,end_date,title):
    ...
    lines=[
        title,
        f"از {start_date} تا {end_date}",
        f"تهیه گزارش: {now().strftime('%Y-%m-%d %H:%M')}",
        "",
        "خلاصه فعالیت‌ها:"
    ]
    ...
    lines += ["", "یادداشت‌های طبقه‌بندی‌شده:"]
...
async def text_handler(update:Update, context:ContextTypes.DEFAULT_TYPE):
    ...
    if action=="reminder_text":
        ...
        await update.message.reply_text(
            picker_text(p, "⏰ ساخت ریمایندر"),
            reply_markup=reminder_picker_menu()
        )
        return
