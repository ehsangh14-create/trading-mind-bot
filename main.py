VERSION_LABEL = "v7_notes_exports_clean"
...
def report_text_for_dates(start_date,end_date,title):
    ...
    notes=get_notes_between(start_date,end_date)
    ...
    if totals:
        for k,sec in totals.items():
            lines.append(f"- {k}: {fmt_duration(sec)}")
    else:
        lines.append("- فعالیتی ثبت نشده است.")

    lines.append("")
    lines.append("یادداشت‌های هر فعالیت:")
    used=set()
    if sessions:
        any_related=False
        for s in sessions:
            ...
            for idx,n in enumerate(notes):
                if note_matches_session(n,s):
                    related.append((idx,n))
            if related:
                any_related=True
                lines.append("")
                lines.append(f"{s['activity_title']} / {s['subject_title']} / {s['sub_title']}:")
                for idx,n in related:
                    used.add(idx)
                    lines.append(f"  - {n['time'][:16]} | {n['text']}")
        if not any_related:
            lines.append("- برای فعالیت‌های این بازه یادداشت مرتبط ثبت نشده است.")
    else:
        lines.append("- فعالیتی در این بازه ثبت نشده است.")

    others=[n for idx,n in enumerate(notes) if idx not in used]
    if others:
        lines.append("")
        lines.append("یادداشت‌های دیگر:")
        grouped=group_notes_by_context(others)
...
async def button_handler(update:Update, context:ContextTypes.DEFAULT_TYPE):
        ...
        if parts[1]=="export":
            ...
            try:
                ...
                with open(path, "rb") as f:
                    await q.message.reply_document(document=f, filename=os.path.basename(path), caption=f"✅ خروجی آماده شد: {os.path.basename(path)}")
    ...
    if data.startswith("export:"):
        _,period,ext=data.split(":")
        e=now().date()
        s=e if period=="today" else e-timedelta(days=6)
        try:
    ...
    if data=="notes:list":
        ...
        if not notes:
            ...
            grouped=group_notes_by_context(notes)
            out=["📝 آخرین یادداشت‌ها:"]
            for label, items in grouped.items():
                out.append("")
                out.append(f"{label}:")
                for n in items:
                    out.append(f"• {n['time'][:16]} - {n['text']}")
            await q.message.reply_text("\n".join(out), reply_markup=life_menu())
        return
