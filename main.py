...
async def button_handler(update:Update, context:ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("range:"):
        ...
        if parts[1]=="export":
            ...
            try: path=export_report(s,e,"گزارش بازه خاص",ext); with open(path, 'rb') as f:
    ...
    if data.startswith("export:"):
        ...
        try: path=export_report(s,e,"گزارش امروز" if period=="today" else "گزارش ۷ روز اخیر",ext); with open(path, 'rb') as f:
                    await q.message.reply_document(document=f, filename=os.path.basename(path), caption="✅ خروجی آماده شد.")
        except Exception as ex: await q.message.reply_text(f"خطا در خروجی:\n{ex}")
        return
