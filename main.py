VERSION_LABEL = "v_reminder_rewrite_5"
...
def life_menu():
    ...
    rows += [
        ...
        [InlineKeyboardButton("⏰ ریمایندر دکمه‌ای", callback_data="rp:open")],
...
async def update_same_message(query, text, reply_markup=None):
    """Update current inline-keyboard message. Never send a new message."""
    try:
...
async def edit_only(query, text, reply_markup=None):
    """Strictly edit the current Telegram message. Never send a new message."""
    try:
        await query.edit_message_text(text=text, reply_markup=reply_markup)
    except Exception:
        try:
...
def clean_picker_get(user_id):
    states = load_states()
    st = states.get(str(user_id), {})
    return st.get("clean_picker")

def clean_picker_save(user_id, picker):
    states = load_states()
    st = states.get(str(user_id), {})
    st["clean_picker"] = picker
    states[str(user_id)] = st
    save_states(states)

def clean_picker_text(p, warning=None):
    txt = (
        "⏰ ساخت ریمایندر\\n\\n"
        f"📅 تاریخ: {p['year']}-{p['month']:02d}-{p['day']:02d}\\n"
...
def clean_picker_menu():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("سال -", callback_data="rp:y:-"), InlineKeyboardButton("سال +", callback_data="rp:y:+")],
        [InlineKeyboardButton("ماه -", callback_data="rp:m:-"), InlineKeyboardButton("ماه +", callback_data="rp:m:+")],
        [InlineKeyboardButton("روز -", callback_data="rp:d:-"), InlineKeyboardButton("روز +", callback_data="rp:d:+")],
        [InlineKeyboardButton("ساعت -", callback_data="rp:h:-"), InlineKeyboardButton("ساعت +", callback_data="rp:h:+")],
        [InlineKeyboardButton("دقیقه -", callback_data="rp:min:-"), InlineKeyboardButton("دقیقه +", callback_data="rp:min:+")],
        [InlineKeyboardButton("📝 نوشتن متن", callback_data="rp:text")],
        [InlineKeyboardButton("✅ ثبت ریمایندر", callback_data="rp:save")],
        [InlineKeyboardButton("⬅️ برگشت", callback_data="root:life")]
    ])

def clean_picker_change(p, field, direction):
    step = 1 if direction == "+" else -1
    ...
    if field == "y":
        ...
        p["hour"] = (p["hour"] + step) % 24

    elif field == "min":
        p["minute"] = (p["minute"] + 5 * step) % 60
