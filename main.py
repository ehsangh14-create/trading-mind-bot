...
from telegram import Update, InlineKeyboardButton, InlineKeyboardMarkup, BotCommand, FSInputFile
...
def mind_menu():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🏴‍☠️ گنج بازار", callback_data="mind:treasure"), InlineKeyboardButton("📊 ستاپ", callback_data="mind:setup")],
        [InlineKeyboardButton("🛡 ریسک", callback_data="mind:risk"), InlineKeyboardButton("🔁 سامسارا", callback_data="mind:samsara")],
...
async def button_handler(update:Update, context:ContextTypes.DEFAULT_TYPE):
    ...
    if data=="range:start": r=range_init(); save_range(q.from_user.id,r); await q.message.reply_text(range_text(r), reply_markup=range_menu()); return
    if data.startswith("range:"):
        parts=data.split(":"); r=get_range(q.from_user.id) or range_init()
        if parts[1]=="focus": r["focus"]=parts[2]; save_range(q.from_user.id,r); await q.message.reply_text(range_text(r), reply_markup=range_menu()); return
        if parts[1] in ["year","month","day"]: r=update_range(r,parts[1],parts[2]); save_range(q.from_user.id,r); await q.message.reply_text(range_text(r), reply_markup=range_menu()); return
        ...
        if parts[1]=="export":
            ...
            try: path=export_report(s,e,"گزارش بازه خاص",ext); await q.message.reply_document(document=FSInputFile(path), caption="✅ خروجی آماده شد.")
    ...
    if data.startswith("export:"):
        ...
        try: path=export_report(s,e,"گزارش امروز" if period=="today" else "گزارش ۷ روز اخیر",ext); await q.message.reply_document(document=FSInputFile(path), caption="✅ خروجی آماده شد.")
        except Exception as ex: await q.message.reply_text(f"خطا در خروجی:\n{ex}")
        return
